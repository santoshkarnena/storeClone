...
class AppStore extends Component {
  state = {selectedCategory: tabsList[0].tabId, inputText: ''}
  ...
  updateSelectedCategory = appCategory => [
    this.setState({selectedCategory: appCategory}),
  ]

  render() {
    const {selectedCategory, inputText} = this.state
    const filterdAppsList = appsList.filter(eachapp => {
      if (
        eachapp.category === selectedCategory &&
        eachapp.appName.toLowerCase().includes(inputText.toLowerCase())
      ) {
        return eachapp
      }
      return ''
        ...
        <div className="app-store">
          ...
          <ul className="tabs-container">
            {tabsList.map(eachTab => (
              <TabItem
                isActive={eachTab.tabId === selectedCategory}
                key={eachTab.tabId}
                tabData={eachTab}
          ...
          <ul className="apps-container">
            {filterdAppsList.map(eachApp => (
              <AppItem key={eachApp.appId} appData={eachApp} />
            ))}
          </ul>
        </div>
      </div>
